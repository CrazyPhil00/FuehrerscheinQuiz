...
text = "Translate this text frm italian to german, only output the number and then the translated german sentence and nothing else:"
# Assuming quiz_data is already populated with your data
for quiz in quiz_data:
    print(f"[{quiz['numero_domanda']}]:{quiz['testo_domanda']} |")
    text += f"[{quiz['numero_domanda']}]:{quiz['testo_domanda']} |"
    i += 1
    if i > 50:
        i = 0
        clipboard.copy(text)
        input()
        text = "Translate this text from italian to german, only output the number and then the translated german sentence and nothing else:"
